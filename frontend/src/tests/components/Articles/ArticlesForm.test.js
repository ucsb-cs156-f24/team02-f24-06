...
describe("ArticlesForm tests", () => {
  ...
  test("renders correctly with no initialContents", async () => {
    ...
    expect(screen.getByTestId("ArticlesForm-submit")).toHaveTextContent("Create");
  });

  test("renders correctly when passing in initialContents", async () => {
    const initialArticle = articlesFixtures.oneArticle;
    renderComponent(
      <ArticlesForm initialContents={initialArticle} submitAction={jest.fn()} />
    );

    expect(await screen.findByTestId("ArticlesForm-id")).toBeInTheDocument();
    expect(screen.getByLabelText(/Id/)).toHaveValue(initialArticle.id.toString());
    ...
    expect(screen.getByLabelText(/Explanation/)).toHaveValue(initialArticle.explanation);
  ...
  test("Correct error messages on missing input", async () => {
    ...
    expect(await screen.findByText(/Date added is required\./)).toBeInTheDocument();
  });
  ...
  test("Max length validation works correctly for Title", async () => {
    ...
    expect(await screen.findByText(/Max length 255 characters\./)).toBeInTheDocument();
  });
  ...
  test("Max length validation works correctly for Explanation", async () => {
    ...
    const explanationField = await screen.findByTestId("ArticlesForm-explanation");
  ...
  test("Max length validation works correctly for Email", async () => {
    ...
    fireEvent.change(emailField, { target: { value: "a".repeat(250) + "@example.com" } });
    fireEvent.click(screen.getByTestId("ArticlesForm-submit"));
